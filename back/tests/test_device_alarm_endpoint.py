...
from models import db, User, Log, DeviceAlarmState, NotificationRule, EmailGroup, SmsGroup
...
def _set_thresholds(client, token, rack, unit, **overrides):
    payload = {
    ...
    return client.put(f'/device-sensors/{rack}/{unit}/thresholds', json=payload,
...
def test_breach_on_get_triggers_non_critical_alarm_and_log(client, app):
    ...
    _set_thresholds(client, token, 'A0', 1, min_temperature=1000, max_temperature=1001)
...
def test_breach_beyond_critical_triggers_both_severities(client, app):
    token = _login(client, app)
    client.get('/device-sensors/A1/1')
    _set_thresholds(client, token, 'A1', 1,
    ...
    with app.app_context():
        assert DeviceAlarmState.is_active('A1', 1, 'temperature', 'non_critical') is True
        assert DeviceAlarmState.is_active('A1', 1, 'temperature', 'critical') is True


def test_repeated_breach_does_not_duplicate_log(client, app):
    token = _login(client, app)
    client.get('/device-sensors/A2/2')
    _set_thresholds(client, token, 'A2', 2, min_temperature=1000, max_temperature=1001)
...
def test_simulate_requires_auth(client):
    resp = client.post('/device-sensors/A0/1/temperature/non_critical/simulate')
    assert resp.status_code == 401


def test_simulate_rejects_unknown_metric(client, app):
    token = _login(client, app)
    resp = client.post('/device-sensors/A0/1/pressure/non_critical/simulate',
                        headers={'Authorization': f'Bearer {token}'})
...
def test_simulate_rejects_unknown_severity(client, app):
    token = _login(client, app)
    resp = client.post('/device-sensors/A0/1/temperature/extreme/simulate',
                        headers={'Authorization': f'Bearer {token}'})
    assert resp.status_code == 400
...
def test_simulate_sets_alarm_active(client, app):
    ...
    resp = client.post('/device-sensors/A3/5/humidity/critical/simulate',
                        headers={'Authorization': f'Bearer {token}'})
    assert resp.status_code == 200
    with app.app_context():
        assert DeviceAlarmState.is_active('A3', 5, 'humidity', 'critical') is True
        assert DeviceAlarmState.is_active('A3', 5, 'humidity', 'non_critical') is False


def test_clear_requires_auth(client):
    resp = client.delete('/device-sensors/A0/1/temperature/non_critical/clear')
    assert resp.status_code == 401


def test_clear_rejects_unknown_metric(client, app):
    token = _login(client, app)
    resp = client.delete('/device-sensors/A0/1/pressure/non_critical/clear',
                          headers={'Authorization': f'Bearer {token}'})
    assert resp.status_code == 400
...
def test_clear_without_active_alarm_returns_404(client, app):
    token = _login(client, app)
    resp = client.delete('/device-sensors/A4/9/temperature/non_critical/clear',
                          headers={'Authorization': f'Bearer {token}'})
...
def test_clear_sets_alarm_inactive_and_logs_user(client, app):
    ...
    client.post('/device-sensors/A5/1/temperature/non_critical/simulate',
                headers={'Authorization': f'Bearer {token}'})

    resp = client.delete('/device-sensors/A5/1/temperature/non_critical/clear',
                          headers={'Authorization': f'Bearer {token}'})
    assert resp.status_code == 200

    with app.app_context():
        assert DeviceAlarmState.is_active('A5', 1, 'temperature', 'non_critical') is False
        logs = Log.get_all_logs()
...
def test_repeated_breach_repeats_after_notify_again_window(client, app):
    token = _login(client, app)
    client.get('/device-sensors/A6/1')
    with app.app_context():
    ...
    _set_thresholds(client, token, 'A6', 1, min_temperature=1000, max_temperature=1001)
    client.get('/device-sensors/A6/1')

    with app.app_context():
        state = DeviceAlarmState.get('A6', 1, 'temperature', 'non_critical')
...
def test_device_alert_uses_custom_sms_text(client, app, monkeypatch):
    ...
    client.get('/device-sensors/A7/1')
    client.post('/device-sensors/A7/1/temperature/non_critical/simulate',
                headers={'Authorization': f'Bearer {token}'})
...
def test_alert_delay_holds_off_first_alarm(client, app):
    token = _login(client, app)
    client.get('/device-sensors/A8/1')
    _set_thresholds(client, token, 'A8', 1, min_temperature=1000, max_temperature=1001,
                     alert_delay_seconds=60)

    client.get('/device-sensors/A8/1')

    with app.app_context():
        assert DeviceAlarmState.is_active('A8', 1, 'temperature', 'non_critical') is False
        state = DeviceAlarmState.get('A8', 1, 'temperature', 'non_critical')
        assert state.pending_since is not None


def test_alert_delay_fires_after_elapsed(client, app):
    token = _login(client, app)
    client.get('/device-sensors/A9/1')
    _set_thresholds(client, token, 'A9', 1, min_temperature=1000, max_temperature=1001,
                     alert_delay_seconds=30)

    client.get('/device-sensors/A9/1')
    with app.app_context():
        state = DeviceAlarmState.get('A9', 1, 'temperature', 'non_critical')
    ...
    with app.app_context():
        assert DeviceAlarmState.is_active('A9', 1, 'temperature', 'non_critical') is True


def test_return_to_normal_logs_once(client, app):
    token = _login(client, app)
    client.get('/device-sensors/A10/1')
    client.post('/device-sensors/A10/1/temperature/non_critical/simulate',
                headers={'Authorization': f'Bearer {token}'})

    _set_thresholds(client, token, 'A10', 1)  # progi w normie -> odczyt wraca w zakres
    client.get('/device-sensors/A10/1')
    client.get('/device-sensors/A10/1')
    ...
    assert len(matching) == 1
    with app.app_context():
        assert DeviceAlarmState.is_active('A10', 1, 'temperature', 'non_critical') is True
...
def test_device_alert_uses_custom_email_subject(client, app, monkeypatch):
    ...
    client.get('/device-sensors/A11/1')
    client.post('/device-sensors/A11/1/temperature/non_critical/simulate',
                headers={'Authorization': f'Bearer {token}'})
...
def test_device_alert_skips_email_outside_schedule(client, app, monkeypatch):
    ...
    client.get('/device-sensors/A12/1')
    client.post('/device-sensors/A12/1/temperature/non_critical/simulate',
                headers={'Authorization': f'Bearer {token}'})
...
def test_device_alert_attaches_camera_snapshot(client, app, monkeypatch):
    ...
    client.get('/device-sensors/A13/1')
    client.post('/device-sensors/A13/1/temperature/non_critical/simulate',
                headers={'Authorization': f'Bearer {token}'})
