...
from models import db, EmailGroup, SmsGroup, DEFAULT_SCHEDULE, is_within_schedule


def test_new_email_group_gets_always_on_schedule(app):
    with app.app_context():
        group = EmailGroup.add_group('IT')
        assert group.schedule == DEFAULT_SCHEDULE


def test_new_sms_group_gets_always_on_schedule(app):
    with app.app_context():
...
def test_update_schedule_persists(app):
    with app.app_context():
        group = EmailGroup.add_group('IT')
        custom = ('0' * 168)
        updated = EmailGroup.update_schedule(group.id, custom)
        assert updated.schedule == custom
        assert EmailGroup.query.get(group.id).schedule == custom


def test_update_schedule_returns_none_for_missing_group(app):
    with app.app_context():
        assert EmailGroup.update_schedule(999, '1' * 168) is None
