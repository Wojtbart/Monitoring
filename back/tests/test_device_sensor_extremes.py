...
def test_first_reading_sets_extremes_to_itself(app):
    device = DeviceSensor.get_or_create_reading('A0')
...
def test_clear_records_resets_to_current_reading(app):
    ...
    updated = DeviceSensor.clear_records('A0')
...
def test_clear_records_returns_none_for_missing_device(app):
    assert DeviceSensor.clear_records('Z9') is None


def test_clear_history_removes_all_rows_for_rack(app):
    for _ in range(5):
        DeviceSensor.get_or_create_reading('A0')
    assert DeviceSensorHistory.query.filter_by(rack_id='A0').count() == 5

    DeviceSensor.clear_history('A0')
    assert DeviceSensorHistory.query.filter_by(rack_id='A0').count() == 0


def test_clear_history_does_not_affect_other_racks(app):
    DeviceSensor.get_or_create_reading('A0')
    DeviceSensor.get_or_create_reading('A1')
    DeviceSensor.clear_history('A0')
    assert DeviceSensorHistory.query.filter_by(rack_id='A1').count() == 1
