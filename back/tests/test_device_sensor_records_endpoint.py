...
def test_clear_records_requires_auth(client):
    resp = client.delete('/device-sensors/A0/records')
    assert resp.status_code == 401


def test_clear_records_404_for_missing_device(client, app):
    token = _login(client, app)
    resp = client.delete('/device-sensors/Z9/records', headers={'Authorization': f'Bearer {token}'})
...
def test_clear_records_resets_extremes(client, app):
    ...
    resp = client.delete('/device-sensors/A0/records', headers={'Authorization': f'Bearer {token}'})
...
def test_clear_history_requires_auth(client):
    resp = client.delete('/device-sensors/A0/history')
    assert resp.status_code == 401
...
def test_clear_history_removes_rows(client, app):
    ...
    resp = client.delete('/device-sensors/A0/history', headers={'Authorization': f'Bearer {token}'})
    assert resp.status_code == 200
    with app.app_context():
        assert DeviceSensorHistory.query.filter_by(rack_id='A0').count() == 0
...
def test_history_range_filters_by_time(client, app):
    ...
    resp = client.get('/device-sensors/A0/history?range=24h')
    data = resp.get_json()['history']
    assert len(data) == 1

    resp_all = client.get('/device-sensors/A0/history?range=month')
    assert len(resp_all.get_json()['history']) == 2

    resp_no_range = client.get('/device-sensors/A0/history')
