...
def test_thresholds_survive_subsequent_reads(app):
    ...
    DeviceSensor.update_thresholds('A0', 10.0, 30.0, 25.0, 70.0, 0.0, 40.0, 15.0, 85.0, 60)
    device = DeviceSensor.get_or_create_reading('A0')
...
def test_update_thresholds_returns_none_for_missing_device(app):
    result = DeviceSensor.update_thresholds('Z9', 10.0, 30.0, 25.0, 70.0, 0.0, 40.0, 15.0, 85.0, 60)
    assert result is None


def test_update_thresholds_persists(app):
    DeviceSensor.get_or_create_reading('A0')
    updated = DeviceSensor.update_thresholds('A0', 12.0, 28.0, 30.0, 60.0, 2.0, 38.0, 20.0, 75.0, 30)
